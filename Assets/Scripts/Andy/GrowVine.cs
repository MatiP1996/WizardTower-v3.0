...
{
    public float growValue = -0.0f; // this value is increased as the player waters the plant
    ...
    void Update()
    {
        ...
        else if (growValue < mainBranchGrowValList[2])// if grow value is in step 1...
        {
        ...
        else if (growValue + lowGrowValOffset < lowerBranchGrowValList[2])// if grow value is in step 1...
        {
        ...
        else if (growValue + midGrowValOffset < midBranchGrowValList[2])// if grow value is in step 1...
        {
        ...
        else if (growValue + topGrowValOffset < topBranchGrowValList[2])// if grow value is in step 1...
        {
        ...
        if (growValue < colliderGrowValList[1])
        {
            SetCollidierScaleAndPosition(1);
        }
        else if (growValue < colliderGrowValList[2])
        {
            SetCollidierScaleAndPosition(2);
        }
    ...
    void SetScaleValue(int stepIndex, List<float> growValList, List<float> scaleValList, Material growMat, float offset)
    {
        ...
        growStepProgress = growValue + offset - growValList[stepIndex - 1];  // get the grow progress in the current step
    ...
    {
        float stepSize = 0.2f;
        growStepProgress = growValue - colliderGrowValList[stepIndex - 1];
        stepProportionComplete = growStepProgress / stepSize;

        Vector3 newPosition = colliderObject.transform.position;
        newPosition.y = Mathf.Lerp(colliderPositionValList[stepIndex - 1], colliderPositionValList[stepIndex], stepProportionComplete);
        colliderObject.transform.position = newPosition;

        Vector3 newScale = colliderObject.transform.localScale;
        newScale.y = Mathf.Lerp(colliderScaleValList[stepIndex - 1], colliderScaleValList[stepIndex], stepProportionComplete);
        colliderObject.transform.localScale = newScale;
    }
}
